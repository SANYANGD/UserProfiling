...
import org.spark_project.dmg.pmml.general_regression.GeneralRegressionModel.ModelType
...
class ProductRecommendationModel {
  ...
  def pr() = {
    ...
    val ratingDF = logsDF.select('global_user_id.as("userId").cast(DataTypes.IntegerType),
