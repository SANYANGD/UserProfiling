...
object cateFeaturedModel {
  ...
  def main(args: Array[String]): Unit = {
    ...
    result = result.select('id.cast(DataTypes.StringType), 'cateFeatured)

    //去重

//    val resultDf = result
//      .withColumn("arrayCol", split(col("cateFeatured"), ","))
//      .withColumn("expCol", explode(col("arrayCol")))
//      .select('id as "memberId", 'expCol as "cateId")
//
//    val resultDuplication = resultDf.except(cateIdDF)
//      .select('memberId as "id", 'cateId as "cateFeatured")
//
//    resultDuplication.show()
//
//    resultDuplication.createOrReplaceTempView("test")
//
//    val Df1 = spark.sql(
//      """
//        |select id,concat_ws(',',collect_set(favorCates)) as favorCates from test group by id
//      """.stripMargin)
//
//    Df1.show()
