...
object ProductRecommendationModel {
  def main(args: Array[String]): Unit = {
    ...
    val result: DataFrame = model.recommendForAllUsers(10)
      .withColumn("GoodsPreference", predict2StringFunc('recommendations))
      .withColumnRenamed("userId", "id")
      .drop('recommendations)
      .select('id.cast(DataTypes.StringType), 'GoodsPreference)
    ...
    def recommendationCatalog =
      s"""{
         ...
         |   "columns":{
         |     "id":{"cf":"rowkey", "col":"id", "type":"string"},
         |     "GoodsPreference":{"cf":"cf", "col":"GoodsPreference", "type":"string"}
         |   }
         |}""".stripMargin
